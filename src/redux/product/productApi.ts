...
import { IDashboardLowProduct } from "@/types/product-type";
...
export const authApi = apiSlice.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    // get 5 products low in stock
    getLowInStock: builder.query<IDashboardLowProduct, number>({
      query: (pos) => `/api/products/dashboard/low-in-stock/${pos}`,
      providesTags: ["DashboardLowInStock"],
      keepUnusedDataFor: 600,
    }),
    // get single product
