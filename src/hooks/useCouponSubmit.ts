...
const useCouponSubmit = () => {
  ...
  const handleCouponSubmit = async (data: any) => {
   ...
   const handleSubmitEditCoupon = async (data: any, id: string, oldCoupon : any) => {
    try {
      const coupon_data = {
        logo: logo ? logo: oldCoupon.logo,
        title: data?.name,
        couponCode: data?.code,
        endTime: dayjs(data.endtime).format('YYYY-MM-DDTHH:mm:ss.SSSZ'),
        discountPercentage: data?.discountpercentage,
        minimumAmount: data?.minimumamount,
        productType: selectProductType ? selectProductType : oldCoupon.productType,
      };
