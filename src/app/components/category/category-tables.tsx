...
const CategoryTables = () => {
  ...
  if (!isLoading && !isError && categories?.result.length === 0) {
    content = <ErrorMsg msg="Không tìm thấy thể loại" />;
  }
    ...
    content = (
      <>
        ...
        {categoryItems.length > countOfPage &&<div className="flex justify-between items-center flex-wrap">
          <p className="mb-0 text-tiny">Hiển thị 1-{categoryItems?.slice(pageStart, pageStart + countOfPage).length} của {categoryItems.length}</p>
