...
const ProductTypeBrand = ({
  ...
  if (isLoading) {
    content = (
      <div className="mb-5">
        <p className="mb-0 text-base text-black">Đang tải...</p>
  ...
  if (!isLoading && isError && brands?.result.length === 0) {
    content = <ErrorMsg msg="Không thấy thể loại" />;
  }
    ...
    content = (
      <div className="mb-5">
        ...
        <Controller
          ...
          control={control}
          rules={{
            required: default_value?.brand ? false : "Thương hiệu không đúng!",
          }}
          render={({ field }) => (
            <ReactSelect
              {...field}
        ...
        <span className="text-tiny leading-4">Đặt thương hiệu sản phẩm.</span>
      </div>
    );
  }
  return (
    <div className="bg-white px-8 py-8 rounded-md mb-6">
      <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-2 gap-x-6">
        ...
        <div className="mb-5">
          ...
          <span className="text-tiny leading-4">Nhập đơn vị sản phẩm.</span>
        </div>
      </div>
    </div>
  );
